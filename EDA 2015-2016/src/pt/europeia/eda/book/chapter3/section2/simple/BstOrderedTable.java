package pt.europeia.eda.book.chapter3.section2.simple;
...
// This version uses the simpler code for the keys() method found in the
// slides.
public class BstOrderedTable<Key extends Comparable<? super Key>, Value> {
    ...
    private boolean isRankConsistent() {
        ...
        if (root != null) {
            ...
            enqueueKeysInRange(keys, minimumNodeOf(root).key,
                    maximumNodeOf(root).key, root);

            for (Key key : keys)
                if (key.compareTo(
                        nodeWithRankIn(rankOf(key, root), root).key) != 0)
                    return false;
        }
