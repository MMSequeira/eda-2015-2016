...
public final class TracingSelection {
    ...
    public static void sort(final double[] values) {
        ...
        for (int numberOfSortedItems = 0; numberOfSortedItems < values.length
            ...
            out.println(
                    "\tPass " + (numberOfPasses + 1)
                            + " selecting minimum for position "
                            + numberOfSortedItems + " from "
                            + (values.length - numberOfSortedItems)
                            + " unobserved items: " + rangeAsString(values,
                                    numberOfSortedItems, values.length - 1)
                            + ":");

            int indexOfMinimum = numberOfSortedItems;
            out.println("\t\tInitial minimum: " + indexOfMinimum);
            for (int i = indexOfMinimum + 1; i != values.length; i++)
                if (isLess(values[i], values[indexOfMinimum])) {
                    indexOfMinimum = i;
                    out.println("\t\tNew minimum: " + indexOfMinimum);
                }
            swap(values, numberOfSortedItems, indexOfMinimum);

            out.println("\tPass result is: " + rangeAsString(values,
                    numberOfSortedItems, values.length - 1) + ":");
    ...
    public static void main(final String[] arguments) {
        final double[] values = { 40, 10, 20, 30 };
